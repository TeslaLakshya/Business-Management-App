...
public class Customerfragment extends Fragment {
    ...
    public static final int USER_INFO_REQUEST_CODE = 15;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v =  inflater.inflate(R.layout.fragment_customerfragment, container, false);
        customerlist = v.findViewById(R.id.customerlist);
        customerlist.setLayoutManager(new LinearLayoutManager(this.getContext()));
        adapter = new CustomerAdapter();
        customerlist.setAdapter(adapter);
        fab = v.findViewById(R.id.floatt);
        viewModel = ViewModelProviders.of(this).get(RashiTrendsViewModel.class);
        viewModel.getAllCustomers().observe(this, new Observer<List<Customers>>() {
            @Override
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == USER_INFO_REQUEST_CODE) {
            if(resultCode == RESULT_OK) {
                Customers customer = new Customers(data.getStringExtra("username")
                , data.getLongExtra("phoneNumber", 0), data.getStringExtra("email")
                , data.getStringExtra("address"), data.getStringExtra("customerSince")
                , data.getByteArrayExtra("userimage"));
